...
# 登录用到的一些URL
# casurl = "https://cas.gzhu.edu.cn/cas_server/login" # 老登录网址
newcasurl = "https://newcas.gzhu.edu.cn/cas/login"
...
def login(session: requests.Session, username: str, password: str) -> bool:
    ...
    result_login = session.get(newcasurl)
    if not is_result_ok(result_login):
        return False
    login_html = etree.HTML(result_login.text)
    login_data = {
        "lt":login_html.xpath("//div[@class='login-tab-details']/input[@name='lt']/@value")[0],
        "execution":login_html.xpath("//div[@class='login-tab-details']/input[@name='execution']/@value")[0],
        "_eventId":login_html.xpath("//div[@class='login-tab-details']/input[@name='_eventId']/@value")[0]
    }
    ...
    result_main = session.post(newcasurl, login_data)
    if not is_result_ok(result_main):
        print_err(f"无法正确访问网页：{newcasurl}")
        return False
...
def clock(session: requests.Session, ymtime: int):
    ...
    # 因为打卡是需要流水号的，需要获取流水号
    result_clock_form = session.post(
        xnyqsb_int_url,
        data={"idc": "XNYQSB", "release": "", "csrfToken": csrfToken, "lang": "zh"},
    )
    if not is_result_ok(result_clock_form):
        print_err(f"在访问{xnyqsb_int_url}时发生错误，状态码为{result_clock_form.status_code}")
        return False
    ...
    formdata_init_data = formdata_init(data, clock_form_url, tags, instanceName, ymtime)
    formdata = (
        str(formdata_init_data)
        .replace('"', '\\"')
        .replace("'", '"')
        .replace(" ", "")
        .replace("False", "false")
    ...
    r = session.post("http://yqtb.gzhu.edu.cn/infoplus/interface/doAction", postdata)
...
def posttext2dingbot(text: str):
    dingbot_buff.append(text)
    # if post2dingbot:
    #     dingbot.post_text(text)


def postmarkdown2dingbot(title: str, text: str):
    dingbot_buff.append(f"{title}{MARKDOWN_NEW_LINE}{text}")
    # if post2dingbot:
...
def main():
    ...
    if isreaddef:
        ...
        postmarkdown2dingbot(
            "【自动打卡】错误",
            f"""<font color=red>{time.strftime("%Y年%m月%d日 %H:%M", time.localtime())}{MARKDOWN_NEW_LINE}{MARKDOWN_NEW_LINE.join(err_buff)}</font>""",
        )
        err_buff.clear()
    else:
        for user in logins:
            try:
                ...
                postmarkdown2dingbot(
                    "【自动打卡】错误",
                    f"""<font color=red>读取某个登录用户失败，配置为：{MARKDOWN_NEW_LINE}{user}</font>""",
                )
                continue
            try:
            ...
            if err_buff:
                postmarkdown2dingbot(
                    "【自动打卡】错误",
                    f"""<font color=red>{time.strftime("%Y年%m月%d日 %H:%M", time.localtime())}时出现错误{MARKDOWN_NEW_LINE}用户名为：{username}{MARKDOWN_NEW_LINE}错误为：{MARKDOWN_NEW_LINE}{MARKDOWN_NEW_LINE.join(err_buff)}{MARKDOWN_NEW_LINE}具体登录后台查看</font>""",
                )
                err_buff.clear()
            else:
                postmarkdown2dingbot(
                    f"""【自动打卡】{username}打卡成功""",
                    f"""{time.strftime("%Y年%m月%d日 %H:%M", time.localtime())}完成用户：{username}的打卡""",
                )
    if post2dingbot:
        import dingbot

        dingbot.post_markdown("自动打卡", MARKDOWN_NEW_LINE.join(dingbot_buff))
